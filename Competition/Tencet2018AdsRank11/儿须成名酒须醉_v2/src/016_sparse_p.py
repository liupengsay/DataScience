...
spar = pd.DataFrame()
for i in range(1,9):
    spar = sparse.hstack((spar,sparse.load_npz('data_preprocessing/train_part_x_sparse_one_'+str(i)+'.npz')))
    print('读到了第',i,'个训练集特征文件')
sparse.save_npz('data_preprocessing/train_part_x_sparse_one_p.npz',spar)
                         
spar = pd.DataFrame()
for i in range(1,9):
    spar = sparse.hstack((spar,sparse.load_npz('data_preprocessing/evals_x_sparse_one_'+str(i)+'.npz')))
    print('读到了第',i,'个验证集特征文件')
sparse.save_npz('data_preprocessing/evals_x_sparse_one_p.npz',spar)
                         
spar = pd.DataFrame()
for i in range(1,9):
    spar = sparse.hstack((spar,sparse.load_npz('data_preprocessing/test2_x_sparse_one_'+str(i)+'.npz')))
    print('读到了第',i,'个测试集特征文件')
sparse.save_npz('data_preprocessing/test2_x_sparse_one_p.npz',spar)